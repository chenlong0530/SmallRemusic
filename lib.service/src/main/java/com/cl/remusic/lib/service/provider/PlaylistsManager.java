package com.cl.remusic.lib.service.provider;
...
import com.cl.remusic.lib.immutable.MusicUtils;
import com.cl.remusic.lib.immutable.info.MusicInfo;
import com.cl.remusic.lib.service.MusicTrack;
...
public class PlaylistsManager {
    ...
    public synchronized void insertMusic(Context context, long playlistid, MusicInfo info) {
        ...
        if (info.islocal) {
            if (albumart.equals(MusicUtils.getAlbumdata(Small.getContext(), info.songId))) {
    ...
    public synchronized void insertLists(Context context, long playlistid, ArrayList<MusicInfo> musicInfos) {
        ...
        for (int i = len - 1; i >= 0; i--) {
            ...
            if (info.islocal) {
                String art = MusicUtils.getAlbumdata(Small.getContext(), info.songId);
                if (art != null) {
                    break;
                } else {
                    albumart = null;
